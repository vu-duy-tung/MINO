...
def create_model_and_transforms(
    ...
    **flamingo_kwargs,
    ...
    clip_model = copy.deepcopy(vision_encoder)
    ...
    text_tokenizer.add_special_tokens(
        {"additional_special_tokens": ["<|endofchunk|>", "<image>", "<graph>"]}
    )
    ...
    model = Flamingo(
        ...
        text_tokenizer.encode("<graph>")[-1],
        vis_dim=open_clip.get_model_config(clip_vision_encoder_path)["vision_cfg"][
            "width"
        ],
        graph_dim=768,
        cross_attn_every_n_layers=cross_attn_every_n_layers,
        enable_graph_input=enable_graph_input,
        **flamingo_kwargs,
    )
    ...
    print(
        ...
        f"and {sum(p.numel() for p in model.parameters())} paramaters in total"
    )

    return model, image_processor, clip_model, text_tokenizer
